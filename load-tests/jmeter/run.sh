...
if [[ "$VALIDATION_MODE" != "true" ]]; then
  ...
  [[ "$WARMUP_SECONDS" == "300" ]] || fail "Experiment 1 warm-up is fixed at 300 seconds"
  [[ "$MEASURED_SECONDS" == "1800" ]] || fail "Experiment 1 measured window is fixed at 1800 seconds"
  [[ "$RUNS" == "3" ]] || fail "Experiment 1 requires exactly three complete runs"
  [[ "$COOLDOWN_SECONDS" == "300" ]] || fail "Experiment 1 requires a 300-second low-load cooldown after every run"
